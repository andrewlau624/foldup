...
const PROVIDERS = [{ envVar: "GROQ_API_KEY", query: queryGroq }];
export async function refold(targetPath, options) {
    if (!targetPath) {
    ...
    if (!activeProvider) {
        console.log(chalk.yellow(`No API key located. Export one of the following:`));
...
async function refoldFolder(folderPath, options, queryFunction, apiKey) {
    ...
    try {
        ...
        if (!plan.moves || plan.moves.length === 0) {
            console.log(chalk.green("This folder is already perfectly folded! No moves suggested."));
            return;
        }
...
function applyMoves(folderPath, moves) {
    moves.forEach((move) => {
...
function printFolderTree(moves) {
    const tree = {};
    moves.forEach((move) => {
